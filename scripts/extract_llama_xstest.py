#!/usr/bin/env python3
"""Extract Llama-3.2-3B-Instruct traces on XSTest 450 (80-tok).

Phase 9 — Different SLM pair (Experiment H).

Outputs:
  results/disagree_routing/phase9_xstest_traces_llama-3.2-3b.json

Saves every 25 prompts; resumes on restart.
Uses existing XSTest prompt set (results/disagree_routing/phase3_xstest_full_meta.json).

Run:
    python3 -u extract_llama_xstest.py
"""
import gc
import json
import os
...
SAVE_PATH = f"{OUT_DIR}/phase9_xstest_traces_llama-3.2-3b.json"
...
def main():
    ...
    for pid, prompt in zip(prompt_ids, prompts):
        ...
        if done_now % 25 == 0 and done_now > 0:
            ...
            print(f"  [{MODEL_NAME}] {len(out)}/{len(prompts)} ({rate:.3f}/s, ~{rem/60:.1f}min left)", flush=True)
