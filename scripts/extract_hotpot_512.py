#!/usr/bin/env python3
"""Extract Qwen + Gemma traces on HotpotQA 250 at 512 tokens.

Phase 10 — Task generalization (HotpotQA correctness boundary).

Re-run of the failed 80-tok pilot (Limitation #7 in main paper). At 512-tok,
SLMs have room to complete multi-hop reasoning, raising baseline accuracy
above the ~2% floor seen at 80-tok and producing enough disagreement
events for AUC estimation.

Outputs:
  results/disagree_routing/phase10_hotpot_traces_qwen3.5-2b.json
  results/disagree_routing/phase10_hotpot_traces_gemma-4-e2b.json
  results/disagree_routing/phase10_hotpot_meta.json
...
PROMPT_FILE = f"{OUT_DIR}/phase2_meta.json"
META_OUT = f"{OUT_DIR}/phase10_hotpot_meta.json"
...
def main():
    ...
    hot = [p for p in bench if p.get("task") == "hotpotqa"]
    print(f"[main] loaded {len(hot)} HotpotQA prompts", flush=True)

    # Save the meta for downstream judge / analysis
    meta = {"prompts": [
        {"id": p["id"], "prompt": p["prompt"], "question": p.get("question", ""),
         "answer": p.get("answer", "")}
        for p in hot
    ]}
    json.dump(meta, open(META_OUT, "w"), ensure_ascii=False)
    print(f"[main] saved meta: {META_OUT}", flush=True)

    prompt_ids = [p["id"] for p in hot]
    prompts = [p["prompt"] for p in hot]
    ...
    for name, model_id in MODELS.items():
        save_path = f"{OUT_DIR}/phase10_hotpot_traces_{name}.json"
        if os.path.exists(save_path):
            try:
                existing = json.load(open(save_path))
                done = existing.get("records", [])
            except Exception:
                done = []
